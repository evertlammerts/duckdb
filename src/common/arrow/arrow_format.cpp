...
ArrowUnit::ArrowUnit() : ResultUnit(ResultUnitType::ARROW) {
}
...
unique_ptr<ArrowUnit> SealAppender(ArrowFormatLocalState &lstate) {
	auto unit = make_uniq<ArrowUnit>();
	unit->row_count = lstate.appender->RowCount();
	// Finalize hands the buffers to the array, so the size has to be read before it
	unit->byte_size = lstate.appender->ByteSize();
...
bool ArrowFormat::IsFull(ResultFormatLocalState &lstate_p) {
	return !lstate_p.Cast<ArrowFormatLocalState>().sealed.empty();
}
